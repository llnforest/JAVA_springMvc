layui.define(['element','colorpicker'],function(exports){
		...
		var type = type || 0;
 		...
 		if(type == 0){
 			layer.open({
 				...
 				btnAlign:'l',
 				btn1:function(){
 					that.submitPassword(that.config.userId);
 				},
 				btn2:function(){
 					layer.closeAll();
 				},
	 ...
	 * @param userId
	 */
    User.prototype.submitPassword = function(userId){
    ...
    User.prototype.selfInfo = function(){
		...
		layer.open({
	    	...
	    	area: ['900px', '600px'],
	        shade: 0.5,
	        content:'/system/user/selfInfo?readonly=true&id='+that.config.userId
	    });
    }
    ...
    User.prototype.setTheme = function(){
		...
		layer.open({
	    	...
	    	btn:["应用","关闭"],
	    	btn1:function(){
	    		that.saveUserTheme(that.config.userId);
	        },
	        btn2:function(){
	           layer.closeAll();
	        },
     ...
     * 保存用户自定义皮肤
     */
    User.prototype.saveUserTheme = function(userId){
        ...
        $.ajax({
            url: '/system/user/setSkin',
            type: 'POST',
            data: {"userId":userId,"topColor":topColor,"leftColor":leftColor},
            dataType: "json",
            success:function (data){
            	 //保存成果
            	 if(data.code=='0'){
